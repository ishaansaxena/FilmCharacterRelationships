...
def build_rmn(d_word, d_char, d_book, d_hidden, len_voc, 
    ...
    in_power_agency = T.fmatrix(name = 'power_agency_chars')

    # TODO add power and agency here
    ...
    l_power_agency = lasagne.layers.InputLayer(shape=(2,2),input_var=in_power_agency)
    # TODO add power and agency here
    ...
    l_mix = MixingLayer([l_dropsum, l_chars,l_power_agency, l_books], #this outputs the hidden state h_t
        d_word, d_char, d_book)


    # compute recurrent weights over dictionary
    l_rels = RecurrentRelationshipLayer(\
        l_mix, d_word, d_hidden, num_descs) #this computes d_t

    # multiply weights with dictionary matrix
    l_recon = ReconLayer(l_rels, d_word, num_descs) #this computes r_t
    ...
    traj_fn = theano.function([in_chars,in_power_agency, in_book,
        in_spans, in_dropmasks], 
        lasagne.layers.get_output(l_rels))
    train_fn = theano.function([in_chars,in_power_agency, in_book,
...
if __name__ == '__main__':
    ...
    with open('data/pa_from_vmaps.csv') as f:
        f.readline()
        pal = list(map(lambda x: x.split(','), f.read().split('\n')))
        power_agency_map = dict((x[0], (float(x[-1]), float(x[-2]))) for x in pal[0:-1])
    ...
    for epoch in range(n_epochs):
        ...
        for book, chars, curr, cm, in span_data:
            ...
            power_char1, agency_char1 = power_agency_map[cmap[chars[0]]]
            power_char2, agency_char2 = power_agency_map[cmap[chars[1]]]
            power_agency_vec = []
            power_agency_vec.append([power_char1,agency_char1])
            power_agency_vec.append([power_char2, agency_char2])
            ex_cost, ex_ortho = train_fn(chars,np.array(power_agency_vec,dtype='float32'), book, curr, cm, drop_mask,
                ns, nm)
            cost += ex_cost
        end_time = time.time()
        ...
        if cost < min_cost:
            ...
            for book, chars, curr, cm in span_data:
                ...
                # TODO: Fix PA File for key errors
                power_char1, power_char2, agency_char1, agency_char2 = \
                    0, 0, 0, 0
                if chars[0] in power_agency_map:
                    power_char1, agency_char1 = power_agency_map[chars[0]]
                if chars[1] in power_agency_map:
                    power_char2, agency_char2 = power_agency_map[chars[1]]
                power_agency_vec = [
                    [power_char1, agency_char1], [power_char2, agency_char2]]

                # feed unmasked inputs to get trajectories
                traj = traj_fn(chars,np.array(power_agency_vec,dtype='float32'), book, curr, cm)
