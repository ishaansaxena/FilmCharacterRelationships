...
class MixingLayer(lasagne.layers.MergeLayer):
    def __init__(self, incomings, d, d_char, 
        ...
        self.W_char = self.add_param(lasagne.init.GlorotUniform(),
            (d, d_char+2), name='W_char') # + 2 to include power and agency.
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        concatenate_power_agency_chars = T.concatenate((inputs[1],inputs[2]),axis =1)
        cvec = T.dot(self.W_char, T.sum(concatenate_power_agency_chars, axis=0))
        bvec = T.dot(self.W_book, T.flatten(inputs[3]))
